...
import { costDepartmentList, costDepartmentAdd, costDepartmentDeleted, costDepartmentEnable, costDepartmentModify } from '@api/costRelation'
...
const costMaintenance = (props) => {
  ...
  const handleOk = async () => {
    if (a === '4') {
      ...
      let res = await costDepartmentEnable(params)
      if (res.success) {
    ...
    if (a === '1') {
      return <Form onFinishFailed={onFinishFailed} onFinish={onFinish}>
        <Row gutter={32}>
          <Col span={12} style={{ width: '30%' }}>
            <Form.Item name="成本部门名称" label="成本部门名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
      ...
      return <div>
        <Form
          ...
          <Row gutter={32}>
            <Col span={12} style={{ width: '30%' }}>
              <Form.Item name="costDepartmentName" label="成本部门名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
      ...
      <ProTable
        ...
        request={async (params, sort, filter) => {
          ...
          let postData = {
            ...
            tellerNo: tellerNo,
            id: params.id,
            costDepartmentName: params.costDepartmentName,
            enable: params.enable,
            beginTime: params.beginTime,
            endDate: params.endDate
          }
          try {
            const res = await costDepartmentList(postData);
