...
import { businessList, businessAdd, businessDeleted, businessEnable, businessModify } from '@api/costRelation'
import { connect } from 'react-redux';
import { cacheUserInfo } from '@state/action/index';
import costMain from '../../../enumeration/costMain';
import moment from 'moment';
const businessMaintenance = (props) => {
  ...
  const handleOk = async () => {
    if (a === '4') {
      ...
      let res = await businessDeleted(params)
      if (res.success) {
      ...
      let res = await businessEnable(params)
      if (res.success) {
  ...
  const onFinish = async () => {
    if (a === '1') {
      let params = {
        tellerNo: tellerNo,
        businessUnitName: addCostName,
        remark: addRemark,
        enable: addStateValue
      }
      const res = await businessAdd(params)
      actionRef.current.reload()
      setAddRemark()
      if (res.success) {
        ...
        openNotification(res.errorMsg ? res.errorMsg : '添加失败')
        setIsModalVisible(false)
      }
    } else if (a === '3') {
      let params = {
        tellerNo: tellerNo,
        businessUnitName: changeCostName,
        remark: changeRemark,
        enable: changeStateValue,
        id: legalId,
      }
      const res = await businessModify(params)
      actionRef.current.reload()
      if (res.success) {
        openNotification('修改成功')
        setIsModalVisible(false);
      } else {
        openNotification(res.errorMsg ? res.errorMsg : '修改失败')
        setIsModalVisible(false)
      }
    }
  };
    ...
    if (a === '1') {
      return <Form onFinishFailed={onFinishFailed} onFinish={onFinish}>
        <Row gutter={32}>
          <Col span={12} style={{ width: '30%' }}>
            <Form.Item name="业务单元名称" label="业务单元名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
      ...
      return <div>
        <Form
          ...
          <Row gutter={32}>
            <Col span={12} style={{ width: '30%' }}>
              <Form.Item name="businessUnitName" label="业务单元名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
  ...
  const costModify = (record) => {
    setRecord(record)
    setChangeCostName(record.businessUnitName)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
          {record.enable === '0' ? <a onClick={() => { look('2'), costEnable(record) }}>启用</ a> : <a onClick={() => { look('2'), costEnable(record) }}>停用</ a>}
    ...
    <div>
      <ProTable
        ...
        request={async (params, sort, filter) => {
          ...
          try {
            const res = await businessList(postData);
...
export default connect(mapStateToProps, mapDispatchToProps)(businessMaintenance);