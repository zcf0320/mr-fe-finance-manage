...
import { businessDepartmentList, businessDepartmentAdd, businessDepartmentDeleted, businessDepartmentEnable, businessDepartmentModify } from '@api/costRelation'
import { connect } from 'react-redux';
import { cacheUserInfo } from '@state/action/index';
import costMain from '../../../enumeration/costMain';
import moment from 'moment';
const departmentMaintenance = (props) => {
  ...
  const handleOk = async () => {
    if (a === '4') {
      ...
      let res = await businessDepartmentDeleted(params)
      if (res.success) {
      ...
      let res = await businessDepartmentEnable(params)
      if (res.success) {
  ...
  const onFinish = async () => {
    if (a === '1') {
      let params = {
        tellerNo: tellerNo,
        businessDepartmentName: addCostName,
        remark: addRemark,
        enable: addStateValue
      }
      const res = await businessDepartmentAdd(params)
      actionRef.current.reload()
      setAddRemark()
      if (res.success) {
      ...
      let params = {
        tellerNo: tellerNo,
        businessDepartmentName: changeCostName,
        remark: changeRemark,
        enable: changeStateValue,
        id: legalId,
      }
      const res = await businessDepartmentModify(params)
      actionRef.current.reload()
      if (res.success) {
      ...
      return <Form onFinishFailed={onFinishFailed} onFinish={onFinish}>
        <Row gutter={32}>
          <Col span={12} style={{ width: '30%' }}>
            <Form.Item name="业务部门名称" label="业务部门名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
      ...
      return <div>
        <Form
          ...
          initialValues={{
            ...
            businessDepartmentName: record.businessDepartmentName,
            remark: record.remark,
          }}
          onFinishFailed={onFinishFailed} onFinish={onFinish}>
          <Row gutter={32}>
            <Col span={12} style={{ width: '30%' }}>
              <Form.Item name="businessDepartmentName" label="业务部门名称" rules={[{ required: true, message: '此项为必填项' }]} style={{ width: '100%' }} >
  ...
  const costModify = (record) => {
    setRecord(record)
    setChangeCostName(record.businessDepartmentName)
      ...
      <ProTable
        ...
        request={async (params, sort, filter) => {
          ...
          let postData = {
            ...
            tellerNo: tellerNo,
            id: params.id,
            businessDepartmentName: params.businessDepartmentName,
            enable: params.enable,
            beginTime: params.beginTime,
            endDate: params.endDate
          }
          try {
            const res = await businessDepartmentList(postData);
...
export default connect(mapStateToProps, mapDispatchToProps)(departmentMaintenance);
